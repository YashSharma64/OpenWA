...
describe('WhatsAppWebJsAdapter honest outcomes (no phantom success)', () => {
  ...
  describe('subscribeToChannel', () => {
    ...
    it('throws ChannelNotFoundError (404) if the invite code cannot be resolved', async () => {
      const getChannelByInviteCode = jest.fn().mockResolvedValue(null);
      const adapter = readyAdapter({ getChannelByInviteCode });
      await expect(adapter.subscribeToChannel('INVITE123')).rejects.toBeInstanceOf(ChannelNotFoundError);
    });
